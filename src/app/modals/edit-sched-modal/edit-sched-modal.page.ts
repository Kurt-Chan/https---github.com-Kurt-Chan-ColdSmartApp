...
export class EditSchedModalPage implements OnInit {
  ...
  ngOnInit() {
    ...
    let schedRef = this.dataService.getSelectedSchedule(
      this.airconId,
      this.schedId
    );
  ...
  validation_messages = {
    schedName: [{ type: 'required', message: 'Id is Required' }],
    setDays: [{ type: 'required', message: 'Password is required.' }],
    startTime: [{ type: 'required', message: 'Aircon Brand is required.' }],
    prefTemp: [{ type: 'required', message: 'Remote Model is required.' }],
    airconMode: [{ type: 'required', message: 'Aircon Mode is required.' }],
  };
